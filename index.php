...
/**
 * 检查 access token（入口验证）
 * 已安装后，所有 API 调用需要先通过 access token 验证
 */
...
function gojs_api_install() {
    ...
    // 设置 session：标记 access token 已验证
    $_SESSION['access_token_valid'] = true;
    ...
    // 返回 bootstrap 数据（包含 access token）
...
function gojs_api_regenerate_access_token() {
    ...
    // 清除当前 session 的验证标记，让用户用新 token 重新访问
    $_SESSION['access_token_valid'] = true; // 保持当前会话有效
...
/**
 * 检查路径是否是 GOJS 系统保护文件/目录
 * 受保护的内容：
 * - .gojs 目录及其所有内容
 * - index.php 主程序文件
 * @param string $full_path 绝对路径
 * @return bool
 */
...
/**
 * 安全解析路径，防止路径遍历攻击
 * 使用 realpath 后验证是否以 root_path 开头
 */
