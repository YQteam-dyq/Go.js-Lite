...
- **统一 REST 契约** — 只保留路径式 `/gojs/api/<action>` 一种路由，旧查询式路由全部移除。
- **文件管理** — 每次保存生成历史快照（可在 `.gojs/file-history/` 找回）；内置图片 / 视频 / 音频 / Markdown 浏览器预览；批量操作；可选 AES-256-GCM 单文件加密。
- **数据库管理** — 持久化连接，慢查询日志。
- **资源监控** — 仪表盘新增 CPU / 内存 / 磁盘趋势图。
- **通知中心** — 在原有邮件 / 钉钉 / 飞书 / Telegram 之外，新增 Microsoft Teams 与 Slack Incoming Webhook 适配器。
- **安全加固** — 暴力破解封禁升级为「IP + UA + 国家」三重判定。
- **运维追溯** — 所有写操作日志都带上 `request_id` / `trace_id`。
...
- �� **资源趋势** — 仪表盘内 CPU / 内存 / 磁盘趋势图 ✅
- ��️ **三重防护** — 暴力破解封禁升级为 IP + UA + 国家三重判定 ✅
- �� **操作追溯** — 所有写操作日志都带 `request_id` / `trace_id` ✅
...
| �� 私密入口 | 带 token 的访问链接，隐藏面板存在 | ✅ |
| �� 仪表盘 | 系统概览、磁盘使用、文件统计、最近修改文件 | ✅ |
| �� 文件管理 | 目录浏览、文件编辑、上传/下载、创建/删除/重命名、权限修改、历史快照、内置预览 | ✅ |
...
| �� 资源趋势 | CPU / 内存 / 磁盘趋势图 | ✅ |
| �� 通知中心 | 邮件 / 钉钉 / 飞书 / Telegram / Microsoft Teams / Slack Incoming Webhook | ✅ |
| �� 操作日志 | 所有写操作日志都带 `request_id` / `trace_id` | ✅ |
